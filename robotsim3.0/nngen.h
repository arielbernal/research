...
struct GENNeuron {
  ...
  void update() {
    Ap = 0;
    for (auto &e : Inputs) v += e.W * e.Input->Ap;
    if (v > theta) {
      Ap = 1;
      v = H;
    }
    if (v > Rp) v = v - D < Rp ? Rp : v - D;
    if (v < Rp) v = v + D > Rp ? Rp : v + D;
  }
...
class GENNeuralNet {
 ...
 protected:
  ...
  void feed(const std::vector<double> &In) {
    for (size_t i = 0; i < Input.size(); ++i) Input[i]->v = In[i];
    for (auto &e : Hidden) e->update();
    for (auto &e : Output) e->update();
  }
