...
public:
  Track() {}
  Track(size_t MazeM, size_t MazeN, float d, unsigned i0 = 0, unsigned j0 = 0)
      : Mz(MazeM, MazeN, i0, j0), DMaze(d) {
  ...
  void getEdgesFromMaze(const Maze &Mz, float d) {
    ...
    for (unsigned i = 0; i < M; ++i) {
      for (unsigned j = 0; j < N; ++j) {
        ...
        if (Cell & WN)
          edges.push_back(Edge2d(x, y, x + d, y));
        if (Cell & WE)
          edges.push_back(Edge2d(x + d, y, x + d, y - d));
        if (Cell & WS)
          edges.push_back(Edge2d(x, y - d, x + d, y - d));
        if (Cell & WW)
          edges.push_back(Edge2d(x, y, x, y - d));
        if (Cell & BN)
          edges.push_back(Edge2d(x, y, x + d, y));
        if (Cell & BE)
          edges.push_back(Edge2d(x + d, y, x + d, y - d));
