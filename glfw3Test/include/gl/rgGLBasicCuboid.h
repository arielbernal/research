...
class GLBasicCuboid : public GLObject {
 public:
  ...
  GLBasicCuboid(const std::string& ObjectName, size_t type = GLCUBOID_STANDARD, const GLMaterial& Material = GLDefaultMaterial)
      : GLObject(ObjectName), Dx(1), Dy(1), Dz(1), type(type), Material(Material) {
    build();
  }
  GLBasicCuboid(const std::string& ObjectName, float dx, float dy, float dz, size_t type = GLCUBOID_STANDARD, const GLMaterial& Material = GLDefaultMaterial)
      : GLObject(ObjectName), Dx(dx), Dy(dy), Dz(dz), type(type), Material(Material) {
      build();
  }
  void build() {
      switch (type) {
