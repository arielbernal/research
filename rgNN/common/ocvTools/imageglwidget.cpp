...
ImageGLWidget::ImageGLWidget(const std::string& Name,
                             cv::Mat& Image,
                             QWidget* parent)
...
void ImageGLWidget::paintGL() {
  ...
  if (FPSEnable) {
    ...
    //    cv::putText(Image, str, cv::Point(600, 25), CV_FONT_HERSHEY_SIMPLEX,
    //    1,
    //                cv::Scalar::all(255));
  }
  if (CallbackRenderer) {
  ...
  glp::renderTexture(TextureId, 0, 0,  Image.cols, Image.rows);
}
