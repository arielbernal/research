...
void SettingsDialog::showPortInfo(int idx) {
  ...
  ui->vidLabel->setText(tr("Vendor Identifier: %1").arg(
      list.count() > 5 ? list.at(5) : tr(blankString)));
  ui->pidLabel->setText(tr("Product Identifier: %1").arg(
      list.count() > 6 ? list.at(6) : tr(blankString)));
}
...
void SettingsDialog::fillPortsInfo() {
  ...
  ui->serialPortInfoListBox->setCurrentIndex(listIndex);
}

void SettingsDialog::updateSettings() {
  currentSettings.name = ui->serialPortInfoListBox->currentText();
  currentSettings.manufacturer = ui->manufacturerLabel->text();
